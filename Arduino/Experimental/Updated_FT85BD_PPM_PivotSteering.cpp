...
// =====================
// Steering mode:
// - Near zero throttle: true pivot / zero-radius turn. Both wheels move opposite directions.
// - At speed: correction steering. One wheel is slowed down instead of boosting the other.

// Increase if pivot turns are too weak. Decrease if pivot turns are too aggressive.
const float PIVOT_STEERING_GAIN = 0.32;

// Minimum command for pivot turns to overcome static friction.
const float MIN_PIVOT_STEER = 0.18;

// Below this throttle magnitude, steering becomes pure pivot.
const float PIVOT_THROTTLE_THRESHOLD = 0.08;

// Above this throttle magnitude, steering becomes mostly speed correction.
const float DRIVE_THROTTLE_THRESHOLD = 0.18;

// Steering at speed: 0.25 = mild, 0.40 = moderate, 0.60 = strong.
// This slows the inside wheel. It does not speed up the outside wheel.
const float SPEED_STEERING_GAIN = 0.35;

// If steering left/right is backwards, flip INVERT_STEERING first.
// If the slowed wheel is wrong while the printed steering direction is correct, flip this.
const bool SWAP_STEERING_SIDES = false;
...
void setup() {
  ...
  Serial.println("FT85BD PPM Mixer: Pivot at zero throttle, slow-inside-wheel at speed");
...
void loop() {
  ...
  // Steering behavior:
  // 1) Near zero throttle: pivot/zero-radius turn.
  //    Master and slave receive equal/opposite commands so both wheels move.
  // 2) At speed: slow the inside wheel only.
  //    This avoids making steering too aggressive by speeding up the outside wheel.

  float masterCmd = 0.0;
  float slaveCmd = 0.0;
  float throttleAbs = abs(throttleCurrent);
  float steeringAbs = abs(steeringCurrent);

  // Optional steering side swap after input inversion.
  float steer = steeringCurrent;
  if (SWAP_STEERING_SIDES) {
    steer = -steer;
  }

  if (throttleAbs < PIVOT_THROTTLE_THRESHOLD) {
    // True zero-radius pivot mode.
    float pivotCmd = steer * PIVOT_STEERING_GAIN;

    if (abs(steer) > 0.10 && abs(pivotCmd) < MIN_PIVOT_STEER) {
      pivotCmd = (pivotCmd >= 0) ? MIN_PIVOT_STEER : -MIN_PIVOT_STEER;
    }

    masterCmd = pivotCmd;
    slaveCmd  = -pivotCmd;
  } else {
    // Drive mode. Steering slows one side instead of speeding the other side.
    float turn = steeringAbs * SPEED_STEERING_GAIN;
    turn = constrain(turn, 0.0, 0.95);

    float outsideCmd = throttleCurrent;
    float insideCmd = throttleCurrent * (1.0 - turn);

    // steer > 0 means one direction, steer < 0 means the other.
    // If the cart turns the wrong way, flip INVERT_STEERING or SWAP_STEERING_SIDES.
    if (steer > 0) {
    ...
    // Blend between pivot and drive behavior for small throttle values.
    if (throttleAbs < DRIVE_THROTTLE_THRESHOLD && steeringAbs > 0.10) {
      float blend = (throttleAbs - PIVOT_THROTTLE_THRESHOLD) /
                    (DRIVE_THROTTLE_THRESHOLD - PIVOT_THROTTLE_THRESHOLD);
      blend = constrain(blend, 0.0, 1.0);

      float pivotCmd = steer * PIVOT_STEERING_GAIN;
      if (abs(pivotCmd) < MIN_PIVOT_STEER) {
        pivotCmd = (pivotCmd >= 0) ? MIN_PIVOT_STEER : -MIN_PIVOT_STEER;
      }

      float pivotMaster = pivotCmd;
      float pivotSlave  = -pivotCmd;

      masterCmd = pivotMaster * (1.0 - blend) + masterCmd * blend;
      slaveCmd  = pivotSlave  * (1.0 - blend) + slaveCmd  * blend;
    }
  }
  ...
  if (millis() - lastPrint > 200) {
    ...
    Serial.print(" | Mode:");
    if (abs(throttleCurrent) < PIVOT_THROTTLE_THRESHOLD) Serial.print("PIVOT");
    else Serial.print("DRIVE");
